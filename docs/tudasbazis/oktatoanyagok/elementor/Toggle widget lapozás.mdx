...
description: Az Elementor toggle widget kiegészítése lapozó és navigációs funkcióval
keywords:
  - elementor
  - toggle widget
  - lapozás
...
be a navigációs nyilakat úgy, hogy a bal oldali, a lapozó előtt, míg a jobb oldali a lapozó után, de ugyanúgy közvetlenül mellette legyen.
```js
 $('.my--left-arrow').insertBefore($('.my--page-link:first-child'));
  $('.my--right-arrow').insertAfter($('.my--page-link:last-child'));
  ```
...
majd ezt a HTML kódot a .my--page-links elembe helyezzük el a .html() metódus segítségével. Ezért nincs szükségünk arra, hogy a HTML-be manuálisan készítsünk
lapozót.
...
Közvetlenül utána máris egy *if* feltétel látható, amelyet arra használunk, hogy ha az adott oldalon a DOM-ban nem található meg a **.mytoggle** class, akkor a kód
nem fut tovább feleslegesen. Egyfelől ezért volt fontos, hogy az elején megadtuk ezt a class-t a toggle widgetnek, továbbá, hogy behatároljuk,
...
Ennek segítségével, amikor lapozunk és szükséges, a látogatót visszagörgeti a toggle-item első elemére. Ez főként akkor hasznos, 
...
Amikor az oldal betöltődik, alapesetben az első oldalon állunk. Ezért mindenképpen szükség van arra, hogy a lapozónk első száma rendelkezzen az active class-szal.
**$('.my--page-link:first-child').addClass('active');** 
...
Ha a kikapcsolt állapotot akarod módosítani, akkor a **.my--pagination-arrow.disabled** kell alkalmazni. Ezen kívül mindkét navigációs nyilat külön class-szal jelöltük.
Ha közvetlenül az ikont szeretnéd módosítani, akkor az elején található HTML-ben kell átírni a FontAwesome-t arra, amire neked van szükséged.
