...
Nem kell aggódnod, ha sokat jelenítesz meg egyszerre, ugyanis a kód automatikusan az első elem tetejére scrollozza vissza a felhasználót.
...
Értelem szerűen erre azért van szükség, hogy egyből tesztelni tudjuk a működést. A példa kódban 9 re van állítva, ezt csak módosítanod kell arra, amennyit
te szeretnél egy oldalon megjeleníteni. 

A toggle widgetnek mindenképpen megkell adnod egy saját class-t, ezt azonosításra fogjuk használni. A saját class-t a szokásos módon a toggle widget haladó fülön tudod megadni
az elrendezés fül legalsó mezőjében a **CSS Osztály**-nál. Itt írd be az alábbi értéket: **mytoggle**

Az elkőkészület utolsó lépése, hogy egy html widgetet a toggle alá beraksz.

## A kód használata

Az alábbi megtalálható kódot teljes terjedelmében másold ki, és illeszd be a html widgetbe. Ha mindent jól csináltál, és több mint 9 toggle elemed van létrehozva,
...
be a navgiációs nyilakat úgy, hogy a bal oldali, a lapozó előtt, míg a jobb oldali a lapozó után, de ugyan úgy közvetlen mellette legyen.
```js
 $('.my--left-arrow').insertBefore($('.my--page-link:first-child'));
  $('.my--right-arrow').insertAfter($('.my--page-link:last-child'));
  ```
...
Közvetlen utána máris egy *if* feltétel látható, amelyt arra használnuk, hogy ha az adott oldalon a DOM ban nem található meg a **.mytoggle** class, akkor a kód
...
elrejti az elemeket, majd a .slice kiválasztja a listából az első 9 elemet, és a .show parancsal megjeleníti azt. 
...
A script továbbá tartalmaz egy kiegészítést is:
 ...
 Ennek segítségével, amikor lapozunk, és szükséges, a látogatót vissza scrollozza a toggle-item első elemére. Ez főként akkor hasznos, 
